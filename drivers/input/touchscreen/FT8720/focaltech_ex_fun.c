...
#define FTS_TP_SELFTEST                             "tp_selftest"
#define FTS_TP_INFO                                 "tp_info"
#define FTS_PROC_TOUCHSCREEN_FOLDER                 "touchscreen"
...
#define PROC_CTP_SAVE_RAWDATA_FILE                  "tp_save_rawdata_info"
#define FTS_PROXIMITY_SWITCH                        "proximity_switch"
#define FTS_STORE_DTAT_PROXIMITY                    "store_data_proximity"


struct proc_dir_entry *fts_proc_tp_selftest_entry = NULL;
struct proc_dir_entry *fts_proc_tp_info_entry = NULL;
struct proc_dir_entry *fts_proc_tp_lockdown_info_file = NULL;
struct proc_dir_entry *fts_proc_touchscreen_dir = NULL;
struct proc_dir_entry *fts_proc_ctp_openshort_test_file = NULL;
struct proc_dir_entry *fts_proc_lockdown_info_file = NULL;
struct proc_dir_entry *fts_proc_save_rawdata_file = NULL;
struct proc_dir_entry *fts_proc_proximity_switch_entry = NULL;
struct proc_dir_entry *fts_proc_store_data_proximity_entry = NULL;
...
extern int wt_fts_test_entry(struct seq_file *m, char *ini_file_name);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_aftersale_test_ops = {
	.proc_read = fts_aftersale_selftest_read,
	.proc_write = fts_aftersale_selftest_write,
};
#else
...
static int fts_tp_lockdown_info_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", "0x53,0x42,0x32,0x01,0x4D,0x13,0x32,0x00");
    return 0;
}

static int fts_tp_lockdown_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, fts_tp_lockdown_info_proc_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_proc_tp_lockdown_info_fops = {
	.proc_open = fts_tp_lockdown_info_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
static const struct file_operations fts_proc_tp_lockdown_info_fops = {
...
static int32_t fts_tp_info_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "[Vendor]Huaxing, [TP-IC]: FT8720,[FW]: 0x%02x\n", fw_version);
	return 0;
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_tp_info_fops = {
...
int wt_fts_factory_test(struct seq_file *m)
{
    int ret = 0;
    char fwname[FILE_NAME_LENGTH] = { "focaltech_ft8720_hx.ini" };
    struct fts_ts_data *ts_data = fts_data;
    ...
    if (ret < 0) {
        ...
        ret = wt_fts_test_entry(m,fwname);
    }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_proc_ctp_openshort_test_fops = {
...
#else
static const struct file_operations fts_proc_ctp_openshort_test_fops = {
...
static int fts_lockdown_info_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", "0x53,0x42,0x32,0x01,0x4D,0x13,0x32,0x00");
    return 0;
}
...
{
    return single_open(file, fts_lockdown_info_proc_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_proc_lockdown_info_fops = {
...
#else
static const struct file_operations fts_proc_lockdown_info_fops = {
	.open 	 = fts_lockdown_info_open,
	.read 	 = seq_read,
	.llseek	 = seq_lseek,
	.release = single_release,
};
...
static int openshort_test_proc_save_show(struct seq_file *m, void *v)
{
    wt_fts_factory_test(m);
    return 0;
}


static int fts_test_save_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, openshort_test_proc_save_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_proc_save_rawdata_fops = {
	.proc_open  = fts_test_save_proc_open,
	.proc_read  = seq_read,
};
#else
static const struct file_operations fts_proc_save_rawdata_fops = {
	.open 	 = fts_test_save_proc_open,
	.read 	 = seq_read,
};
#endif
...
int32_t fts_set_proximity_switch(uint8_t proximity_switch)
{
	int ret = 0;
	   u8 buf_addr = 0;
	   u8 buf_value = 0;
...
static ssize_t fts_proximity_switch_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	uint8_t proximity_switch;
	...
	if (count == 0 || count > 2) {
		FTS_ERROR("Invalid value!, count = %zu\n", count);
		ret = -EINVAL;
		goto out;
	}
	...
	if (!tmp_buf) {
		FTS_ERROR("Allocate tmp_buf fail!\n");
		ret = -ENOMEM;
		goto out;
	}
	if (copy_from_user(tmp_buf, buf, count)) {
		FTS_ERROR("copy_from_user() error!\n");
		ret = -EFAULT;
		goto out;
	}
	...
	if (ret != 1) {
		FTS_ERROR("Invalid value!, ret = %d\n", ret);
		ret = -EINVAL;
		goto out;
	}
	if ((tmp < 0) || (tmp > 1)) {
		FTS_ERROR("Invalid value!, tmp = %d\n", tmp);
		ret = -EINVAL;
		goto out;
	}
	proximity_switch = (uint8_t)tmp;
	FTS_INFO("proximity_switch = %d\n", proximity_switch);
	...
	mutex_lock(&input_dev->mutex);
	fts_set_proximity_switch(proximity_switch);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_proximity_switch_fops = {
	.proc_read = fts_proximity_switch_proc_read,
	.proc_write = fts_proximity_switch_proc_write,
};
#else
static const struct file_operations fts_proximity_switch_fops = {
	.read  = fts_proximity_switch_proc_read,
	.write = fts_proximity_switch_proc_write,
};
#endif

static ssize_t fts_store_data_proximity_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	int ret = 0;
	int cnt = 0;
	char *data_buf;
	...
	if (!data_buf) {
		FTS_INFO("Allocate tmp_buf fail!\n");
		ret = -ENOMEM;
		kfree(data_buf);
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops fts_store_data_proximity_fops = {
	.proc_read = fts_store_data_proximity_read,
	.proc_poll = fts_store_data_proximity_poll,

};
#else
static const struct file_operations fts_store_data_proximity_fops = {
	.read = fts_store_data_proximity_read,
	.poll = fts_store_data_proximity_poll,
};
#endif

int fts_proc_node_init(void)
{
    fts_proc_tp_selftest_entry = proc_create(FTS_TP_SELFTEST, (S_IWUSR | S_IRUGO), NULL, &fts_aftersale_test_ops);
	if (fts_proc_tp_selftest_entry == NULL) {
        printk(KERN_ERR "%s: fts_proc_tp_selftest_entry file create failed!\n", __func__);
	} else {
		printk("%s:  proc_create fts_proc_tp_selftest_entry success",__func__);
	}

	fts_proc_tp_info_entry = proc_create(FTS_TP_INFO, (S_IWUSR | S_IRUGO), NULL, &fts_tp_info_fops);
	if (fts_proc_tp_info_entry == NULL) {
		printk(KERN_ERR "%s: fts_proc_tp_info_entry file create failed!\n", __func__);
		remove_proc_entry(FTS_TP_INFO, NULL);
	}else{
		printk("%s:  proc_create fts_proc_tp_info_entry success",__func__);
	}

	fts_proc_tp_lockdown_info_file = proc_create(PROC_CTP_LOCKDOWN_INFO_FILE, (S_IWUSR | S_IRUGO), NULL, &fts_proc_tp_lockdown_info_fops);
	if (fts_proc_tp_lockdown_info_file == NULL) {
		printk(KERN_ERR "%s: fts_proc_tp_info_entry file create failed!\n", __func__);
    ...
    if (fts_proc_touchscreen_dir == NULL)  {
        ...
        if(fts_proc_ctp_openshort_test_file ==NULL){
            ...
            remove_proc_entry(FTS_PROC_CTP_OPENSHORT_TEST_FILE, NULL);
        }else{
            printk("%s:  proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
        }
        fts_proc_lockdown_info_file = proc_create(PROC_LOCKDOWN_INFO_FILE,
            (S_IWUSR | S_IRUGO), fts_proc_touchscreen_dir, &fts_proc_lockdown_info_fops);
        if (fts_proc_lockdown_info_file == NULL) {
            printk (KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
            remove_proc_entry(PROC_LOCKDOWN_INFO_FILE, NULL);
            } else {
                printk ("%s:  proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
            }
		fts_proc_save_rawdata_file = proc_create(PROC_CTP_SAVE_RAWDATA_FILE,
			(S_IWUSR | S_IRUGO), fts_proc_touchscreen_dir, &fts_proc_save_rawdata_fops);
		if (fts_proc_save_rawdata_file == NULL) {
			printk (KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
			remove_proc_entry(PROC_CTP_SAVE_RAWDATA_FILE, NULL);
			} else {
				printk ("%s:  proc_create PROC_CTP_SAVE_RAWDATA_FILE success",__func__);
			}
		fts_proc_proximity_switch_entry = proc_create(FTS_PROXIMITY_SWITCH,
			(S_IWUSR | S_IRUGO), fts_proc_touchscreen_dir, &fts_proximity_switch_fops);
		if (fts_proc_proximity_switch_entry == NULL) {
			printk (KERN_ERR "%s: FTS_proc_proximity_switch create failed!\n", __func__);
			remove_proc_entry(FTS_PROXIMITY_SWITCH, NULL);
		}
		fts_proc_store_data_proximity_entry = proc_create(FTS_STORE_DTAT_PROXIMITY,
			(S_IWUSR | S_IRUGO), fts_proc_touchscreen_dir, &fts_store_data_proximity_fops);
		if (fts_proc_store_data_proximity_entry == NULL) {
			printk (KERN_ERR "%s: FTS_proc_store_data_proximity create failed!\n", __func__);
			remove_proc_entry(FTS_STORE_DTAT_PROXIMITY, NULL);
		}
	}

    return  0;
}
void fts_proc_node_deinit(void)
{
	if (fts_proc_tp_selftest_entry != NULL) {
		remove_proc_entry(FTS_TP_SELFTEST, NULL);
		fts_proc_tp_selftest_entry = NULL;
	...
	if (fts_proc_tp_lockdown_info_file != NULL) {
		remove_proc_entry(PROC_CTP_LOCKDOWN_INFO_FILE, NULL);
		fts_proc_tp_lockdown_info_file = NULL;
		FTS_INFO("Removed /proc/%s\n", PROC_CTP_LOCKDOWN_INFO_FILE);
	}

	if (fts_proc_ctp_openshort_test_file != NULL) {
		remove_proc_entry(FTS_PROC_CTP_OPENSHORT_TEST_FILE, NULL);
		fts_proc_ctp_openshort_test_file = NULL;
		FTS_INFO("Removed /proc/%s\n", FTS_PROC_CTP_OPENSHORT_TEST_FILE);
	}

	if (fts_proc_lockdown_info_file != NULL) {
		remove_proc_entry(PROC_LOCKDOWN_INFO_FILE, NULL);
		fts_proc_lockdown_info_file = NULL;
		FTS_INFO("Removed /proc/%s\n", PROC_LOCKDOWN_INFO_FILE);
	}

	if (fts_proc_save_rawdata_file != NULL) {
		remove_proc_entry(PROC_CTP_SAVE_RAWDATA_FILE, NULL);
		fts_proc_save_rawdata_file = NULL;
		FTS_INFO("Removed /proc/%s\n", PROC_CTP_SAVE_RAWDATA_FILE);
	}

	if (fts_proc_proximity_switch_entry != NULL) {
		remove_proc_entry(FTS_PROXIMITY_SWITCH, NULL);
		fts_proc_proximity_switch_entry = NULL;
		FTS_INFO("Removed /proc/%s\n", FTS_PROXIMITY_SWITCH);
	}

	if (fts_proc_store_data_proximity_entry != NULL) {
		remove_proc_entry(FTS_STORE_DTAT_PROXIMITY, NULL);
		fts_proc_store_data_proximity_entry = NULL;
		FTS_INFO("Removed /proc/%s\n", FTS_STORE_DTAT_PROXIMITY);
	}

	if (fts_proc_touchscreen_dir != NULL) {
		remove_proc_entry(FTS_PROC_TOUCHSCREEN_FOLDER, NULL);
		fts_proc_touchscreen_dir = NULL;
		FTS_INFO("Removed /proc/%s\n", FTS_PROC_TOUCHSCREEN_FOLDER);
	}
